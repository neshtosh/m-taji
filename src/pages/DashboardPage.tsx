  ...
  const renderProfileContent = () => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Activity & Metrics</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3">
              <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{dashboardMetrics.totalBlogPosts}</div>
              <div className="text-xs text-gray-600 dark:text-gray-400">Blog Posts</div>
            </div>
            <div className="bg-pink-50 dark:bg-pink-900/20 rounded-lg p-3">
              <div className="text-2xl font-bold text-pink-600 dark:text-pink-400">{dashboardMetrics.totalMicroblogPosts}</div>
              <div className="text-xs text-gray-600 dark:text-gray-400">Microblog Posts</div>
            </div>
          </div>

          {/* Fundraising Progress */}
          {dashboardMetrics.totalFundraisingCampaigns > 0 && (
            <div className="mb-6">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Fundraising Progress</h4>
              <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Total Raised</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-white">
                    KSh {dashboardMetrics.totalFundsRaised.toLocaleString()} / KSh {dashboardMetrics.totalFundraisingGoal.toLocaleString()}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, (dashboardMetrics.totalFundsRaised / Math.max(dashboardMetrics.totalFundraisingGoal, 1)) * 100)}%` 
                    }}
                  ></div>
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {dashboardMetrics.totalFundraisingCampaigns} active campaigns
                </div>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-3">
              {projects.slice(0, 3).map((project: ProjectWithCampaign) => (
                <div key={project.id} className="flex items-center space-x-3 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
                  <img 
                    src={project.image_url || 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&h=300&fit=crop'} 
                    alt={project.title}
